...
Eigen::VectorXd KDLController::idCntr(KDL::Frame &_desPos,  KDL::Twist &_desVel,  KDL::Twist &_desAcc,  double _Kpp,  double _Kpo,  double _Kdp,  double _Kdo)
{
    ...
    Eigen::VectorXd grad = gradientJointLimits(robot_->getJntValues(),robot_->getJntLimits());
...
//    std::cout << "---------------------" << std::endl;
    ...
    return B*(Jpinv*y + (I - Jpinv*J)*(1*grad /*+ robot_->getJntVelocities()*/)) + robot_->getGravity() + robot_->getCoriolis() + robot_->getFrictions();
